...
export default function PurchaseInvoice() {
  ...
  const handleBarcodeScan = (scannedBarcode: string) => {
    const med = medicines.find(m => (m.barcode === scannedBarcode || m.code === scannedBarcode) && m.branchId === selectedBranch?.id);
    if (med) {
      addItem(med);
      setBarcode('');
    }
  ...
  return (
    <div className="flex h-full bg-[#f0f0f0]">
      ...
      <div className="w-16 bg-gray-200 border-l border-gray-300 flex flex-col items-center py-4 gap-4">
        ...
        <button type="button" onClick={() => navigate('/purchases/new')} className="p-2 bg-gray-300 rounded border-2 border-orange-400" title="المشتريات"><Radio className="w-8 h-8 text-orange-600" /></button>
        <button type="button" onClick={() => navigate('/pos')} className="p-2 hover:bg-gray-300 rounded" title="المبيعات"><TrendingUp className="w-8 h-8 text-green-700" /></button>
