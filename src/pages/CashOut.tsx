...
import { ArrowDownRight, Save } from 'lucide-react';
...
export default function CashOut() {
  ...
  const fetchAccounts = async () => {
    try {
      ...
      setAccounts(data.filter(a => a.type === 'OUT'));
    } catch (error) {
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="lg:col-span-1 bg-white p-4 rounded shadow-sm border border-gray-200 h-fit">
          <h2 className="text-lg font-bold text-gray-800 border-b pb-2 mb-4">تسجيل عملية صرف</h2>
        ...
        <div className="lg:col-span-2 bg-white rounded shadow-sm border border-gray-200 overflow-hidden">
          <div className="p-4 border-b border-gray-200 bg-gray-50">
            <h2 className="text-lg font-bold text-gray-800">سجل المصروفات</h2>
          </div>
              ...
              <tbody className="divide-y divide-gray-200">
                {accounts.map((acc) => (
                  <tr key={acc.id} className="hover:bg-gray-50">
                    ...
                    <td className="p-3 font-bold text-red-600">{acc.amount.toFixed(2)}</td>
