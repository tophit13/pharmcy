...
export default function PurchaseReturn() {
  ...
  const handleSelectPurchase = (id: number | '') => {
    ...
    if (id) {
      ...
      if (purchase) {
        setReturnItems(purchase.items.map(item => ({ id: item.medicineId, quantity: 0 })));
      }
    } else {
      ...
      <div className="bg-white p-4 rounded shadow-sm border border-gray-200">
        <div className="max-w-md">
          ...
          <select
            ...
            onChange={(e) => handleSelectPurchase(e.target.value ? parseInt(e.target.value) : '')}
          >
            <option value="">-- اختر فاتورة الشراء --</option>
            {purchases.map(p => (
              <option key={p.id} value={p.id}>
                فاتورة رقم {p.id} - {format(new Date(p.date), 'yyyy-MM-dd HH:mm')} - {p.netTotal} ج.م
              </option>
            ))}
          </select>
        </div>
      ...
      {selectedPurchase && (
        <div className="bg-white rounded shadow-sm border border-gray-200 overflow-hidden">
          <div className="p-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
            <h2 className="text-lg font-bold text-gray-800">تفاصيل الفاتورة رقم {selectedPurchase.id}</h2>
            <div className="text-sm text-gray-600">
              التاريخ: {format(new Date(selectedPurchase.date), 'yyyy-MM-dd HH:mm')}
            </div>
          </div>
            ...
            <table className="w-full text-right text-sm">
              <thead className="bg-gray-100 text-gray-700">
                <tr>
                  ...
                  <th className="p-3 font-medium">الكمية المشتراة</th>
